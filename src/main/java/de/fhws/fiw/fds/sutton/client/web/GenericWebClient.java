...
public class GenericWebClient<T extends AbstractClientModel>
{
	...
	{
		return sendGetSingleRequest( url, new Headers.Builder( ).build( ) );
	}

	public WebApiResponse<T> sendGetSingleRequest( final String url, Headers headers ) throws IOException
	{
		final Response response = executeGetRequest( url, headers );
	...
	private Response executeGetRequest( final String url, Headers headers ) throws IOException
	{
	...
	public WebApiResponse<T> sendGetSingleRequest( final String url, final Class<T> clazz )
		throws IOException
	{
		return sendGetSingleRequest( url, clazz, new Headers.Builder( ).build( ) );
	}

	public WebApiResponse<T> sendGetSingleRequest( final String url, final Class<T> clazz, final Headers headers )
		throws IOException
	{
		final Response response = executeGetRequest( url, headers );
	...
	public WebApiResponse<T> sendGetCollectionRequest( final String url,
		final Class<T> clazz, Headers headers ) throws IOException
	{
	...
	{
		return sendGetCollectionRequest( url, clazz, new Headers.Builder( ).build( ) );
	}
	 ...
	 *                     Because networks can fail during an exchange, it is possible that the remote server accepted the request before the failure.
	 */
	public WebApiResponse<T> sendPostRequest( final String url, final T object, Headers headers ) throws IOException
	{
	...
	{
		return sendPostRequest( url, object, new Headers.Builder( ).build( ) );
	}
	 ...
	 *                     Because networks can fail during an exchange, it is possible that the remote server accepted the request before the failure.
	 */
	public WebApiResponse<T> sendPutRequest( final String url, final T object, Headers headers ) throws IOException
	{
		final RequestBody body =
			RequestBody.create( MediaType.parse( APPLICATION_JSON ), serialize( object ) );

		final Request request = new Request.Builder( ).url( url ).headers( headers ).put( body ).build( );
	...
	{
		return sendPutRequest( url, object, new Headers.Builder( ).build( ) );
	}
	 ...
	 *                     Because networks can fail during an exchange, it is possible that the remote server accepted the request before the failure.
	 */
	public WebApiResponse<T> sendDeleteRequest( final String url, Headers headers ) throws IOException
	{
		final Request request = new Request.Builder( ).url( url ).headers( headers ).delete( ).build( );
	...
	{
		return sendDeleteRequest( url, new Headers.Builder( ).build( ) );
	}
